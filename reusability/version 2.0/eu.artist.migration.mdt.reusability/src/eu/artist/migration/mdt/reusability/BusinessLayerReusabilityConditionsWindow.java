...
public class BusinessLayerReusabilityConditionsWindow {
	...
	public static void main(String[] args) {
		try {
			String strConfigurationFilePath = "D:/ARTIST/DEMO/config.xml";
		...
		File folder = new File("D:/ARTIST/DEMO/agoncal-petstore-ee6-eclipse");				
		Searcher s= new Searcher();		
		ProgrammingLanguage programmingLanguage = new ProgrammingLanguage();
		programmingLanguage.setFileExt("java");
		...
		s.addType(programmingLanguage.getFileExt());		
				...
				DataStorageFramework dataStorageFramework = mapDataStorageFrameworks.get("JPA");
		...
		for (SearchResult sr : s.getNoResult()) {
			...
			try {
				p.addSourceFile(ce.findMethodsParser(sampleFile,reusabilityLevel));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		for (SearchResult sr : s.getResult()) {
			...
			System.out.println(sr.getFileName());
			try {
				p.addSourceFile(ce.findMethodsParser(sampleFile, reusabilityLevel,sr.getLines()));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		try {    		
			MarshallCode.marshallProjectXML(p, "D:/ARTIST/DEMO/target/output.xml");
		} catch (Exception e1) {						
			e1.printStackTrace();
		}*/
	}
	...
	protected void createContents() {
		...
		btnSameTechDataStorageYes.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				//grpSameProductFor.setVisible(false);
				grpSameDatastorageFramework.setVisible(btnSameTechDataStorageYes.getSelection());				
		...
		grpSameDatastorageFramework.setText("Same framework for data storage on source and destination:");
		grpSameDatastorageFramework.setBounds(45, 256, 350, 96);
		
		btnSameDatastorageFrameworkYes = new Button(grpSameDatastorageFramework, SWT.RADIO);
		btnSameDatastorageFrameworkYes.setText("Yes");
		btnSameDatastorageFrameworkYes.setBounds(25, 20, 83, 16);
		
		btnSameDatastorageFrameworkNo = new Button(grpSameDatastorageFramework, SWT.RADIO);
		btnSameDatastorageFrameworkNo.setText("No");
		btnSameDatastorageFrameworkNo.setBounds(25, 42, 83, 16);
		
		labelDatastorageFrameworkOnSource = new Label(grpSameDatastorageFramework, SWT.NONE);
		labelDatastorageFrameworkOnSource.setText("Technology on source:");
		labelDatastorageFrameworkOnSource.setBounds(25, 66, 133, 20);
		
		comboDatastorageFrameworkOnSource = new Combo(grpSameDatastorageFramework, SWT.NONE);
		...
		while (iterator.hasNext()){
			DataStorageFramework dataStorageFramework = (DataStorageFramework) iterator.next();		
			comboDatastorageFrameworkOnSource.add(dataStorageFramework.getName());
		}		
		comboDatastorageFrameworkOnSource.select(0);		
	...
	public void setReusableMethods(){
		...
		if (btnSameTechDataStorageNo.getSelection()){		
			...
			if (btnSameDatastorageFrameworkNo.getSelection()){
				HashMap<String, DataStorageFramework> mapDataStorageFrameworks = Configuration.mapDatastorageFrameworks;
				DataStorageFramework dataStorageFramework = mapDataStorageFrameworks.get(comboDatastorageFrameworkOnSource.getText());
				reusabilityLevel=dataStorageFramework.getReusabilityLevel();
				ArrayList<?> keywords = dataStorageFramework.getKeywords();
				Iterator<?> iterator = keywords.iterator();
				while (iterator.hasNext()){
